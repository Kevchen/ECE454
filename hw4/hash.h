...
template<class Ele, class Keytype>
void
hash<Ele,Keytype>::merge(hash* copyhash){
	  unsigned i,j;

	  //walk through hash table
	  for (i=0;i<my_size;i++){
		//walk through each list
		Ele* temp = copyhash->entries[i].pop();
		while(temp != NULL){
			Ele* sample;
			if(!(sample = lookup(temp->key()))){
				insert(temp);
			}
			else{
				sample->count += temp->count;
			}
			temp = copyhash->entries[i].pop();
		}
