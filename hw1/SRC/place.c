		   ...
		   int num_regions)
{
	...
	{
    	//daniel test
//    	struct s_place_region *place_region_xi = place_region_x[i];
//    	struct s_place_region *place_region_yi = place_region_y[i];
//
//    	float *old_region_occ_xi = old_region_occ_x[i];
//    	float *old_region_occ_yi = old_region_occ_y[i];
		  ...
		  int num_regions)
{
    ...
    for(i = imin; i <= imax; i++)
	{
    	...
    	struct s_place_region *place_regionxi = place_region_x[i];
    	struct s_place_region *place_regionyi = place_region_y[i];

	    for(j = jmin; j <= jmax; j++)
		{
		    overlap_xlow = max(place_region_bounds_x[i], net_xmin);
		    overlap_xhigh =
			min(place_region_bounds_x[i1], net_xmax);
		    ...
		    //daniel test
		    float crossyx = crossing * y_overlap * x_overlap;

		    x_occupancy =
		    		crossyx * inv_bb_height *
			inv_region_len;
		    y_occupancy =
		    		crossyx * inv_bb_len *
			inv_region_height;

		    //daniel test replace array [i]
		    place_regionxi[j].occupancy +=
			add_or_sub * x_occupancy;
		    place_regionyi[j].occupancy +=
		      ...
		      struct s_bb *bb_coord_new)
{
    ...
    //daniel test
    int numsinks1 = net[inet].num_sinks + 1;

    for(k = 1; k < numsinks1; k++)
	{
